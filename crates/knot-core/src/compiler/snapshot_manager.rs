...
impl SnapshotManager {
    ...
    pub fn restore_if_needed(
        ...
        let current_loaded = self.loaded_snapshot_per_lang.get(lang).cloned().unwrap_or_default();
        ...
        if current_loaded != previous_hash {
            ...
            if snapshot_path.exists() {
                ...
                exec.load_session(&snapshot_path)
                    .context(format!("Failed to restore {} session snapshot for hash {}", lang, &previous_hash[..8]))?;
                ...
                for (obj_name, info) in &cache.metadata.constant_objects {
                    if info.language == lang {
                        exec.load_constant(obj_name, &info.hash, &cache_dir)
                            .context(format!("Failed to load constant '{}' into {} environment", obj_name, lang))?;
                    }
                }
                
                self.loaded_snapshot_per_lang.insert(lang.to_string(), previous_hash.to_string());
            }
        }
    ...
    pub fn update_after_node(
        ...
        if !snapshot_exists {
            ...
            self.loaded_snapshot_per_lang.insert(lang.to_string(), node_hash.to_string());
